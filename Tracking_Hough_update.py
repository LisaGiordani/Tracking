...
maximum = "Classic" # method to get the maximum of the Hough transform : "Classic" or "MeanShift"
update_RTable = False # update R-Table for every new frame
...
while(1):
    ...
    if ret == True:
        ...
        if maximum == "Classic":
            ...
            ret = True
        if maximum == "MeanShift":
            ret, track_window = cv2.meanShift(H, track_window, term_crit)
            window_r, window_c, window_h, window_w = track_window
        #---------------------------------------------------------------------#
